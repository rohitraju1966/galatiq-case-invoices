...
def extract_invoice(state: InvoiceState) -> dict:
    ...
    for attempt in range(MAX_EXTRACTION_ATTEMPTS):
        ...
        logger.warning(f"Extraction attempt {attempt + 1}/{MAX_EXTRACTION_ATTEMPTS} failed: {last_error}")

    if response is None:
        raise ValueError(f"Extraction failed after {MAX_EXTRACTION_ATTEMPTS} attempts: {last_error}")
