...
class InvoiceDB:
    ...
    def is_duplicate(self, invoice_number: str, current_trn_id: int) -> bool:
        return (
            self.session.query(TransactionAuditLogs)
            .join(
                TransactionInvoices,
    ...
    def get_item(self, item_name: str) -> tuple[MasterInventory | None, bool]:
        direct = (
            self.session.query(MasterInventory)
            .filter(
    ...
    def get_merchant(self, merchant_name: str) -> MasterMerchant | None:
        return (
            self.session.query(MasterMerchant)
            .filter(
    ...
    def get_invoice_history(self, merchant_name: str, current_trn_id: int) -> list:
        # Excludes the invoice under review (inserted at extraction, before the VP runs).
        return (
            self.session.query(TransactionInvoices)
            .filter(
    ...
    def get_audit_trail(self, trn_id: int) -> list[TransactionAuditLogs]:
        return (
            ...
            .order_by(TransactionAuditLogs.audit_log_id)
            .all()
        )
    ...
    def get_items(self, trn_id: int) -> list[TransactionInvoiceItems]:
        return (
            self.session.query(TransactionInvoiceItems)
            .filter(
